(function(){
  ...
  function installDogProfileBanner(){
    ...
    const brand = topnav && topnav.querySelector('.brand');
    const footerBrowse = document.querySelector('footer a[href$="browse-trails.html"]');
    const contextHref = (brand && brand.getAttribute('href'))
      || (footerBrowse && footerBrowse.getAttribute('href'))
      || 'index.html';
    const prefix = contextHref.startsWith('/') ? '/' : (contextHref.startsWith('../') ? '../' : '');
