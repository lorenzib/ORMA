(function(){
  ...
  function renderNext(model){
    ...
    if(failure){owner.textContent='Automation blocked';owner.className='bo-owner-chip is-system';set('nextHandoffTitle',`${failure.targetTrailId||failure.candidateId||'Trail'} publication did not complete`);set('nextHandoffCopy',`The release stopped at ${(failure.failureStage||'publication automation').replace(/-/g,' ')}. ${failure.failureMessage||'A durable failure receipt was saved.'}`);set('nextHandoffAfter','Your approval is retained. Do not approve again; ORMA can safely retry the same release after the problem is corrected.');const href=failure.pullRequestUrl||failure.workflowRunUrl;if(href){action.href=href;action.textContent=failure.pullRequestUrl?'Open publication PR ↗':'Inspect failed automation run ↗';action.target='_blank';action.rel='noopener';action.hidden=false;}else action.hidden=true;return;}
    ...
    if(model.handoffsInFlight){owner.textContent='Saved in Firestore';owner.className='bo-owner-chip is-system';set('nextHandoffTitle','ORMA automation owns the next step');set('nextHandoffCopy',`${model.handoffsInFlight} saved decision${model.handoffsInFlight===1?' is':'s are'} queued for the secure cloud process that runs every five minutes.`);set('nextHandoffAfter','You may close this page. Do not click again; the trail will return only if another decision is required.');action.hidden=true;return;}
  ...
  function activityTitle(item){
    ...
    const stream={dossier:'Evidence decision',content:'Trail content decision',publication:'Release decision'}[item.stream]||'Trail decision';
  ...
  function render(model){
    ...
    renderNext(model);renderPipeline(model);renderDecisions(model);renderActivity(model);
  }
  async function load(){
    ...
    try{const remote=await api();const [orchestration,dossiers,execution,publication,publicationRequests,jobResult,historyResult]=await Promise.all([
      required(remote,'trail-orchestration'),required(remote,'dossier-review-queue'),required(remote,'verified-trail-editorial-execution'),required(remote,'publication-staging'),optional(remote,'publication-requests',{requests:[]}),remote.getRevisionJobs(),remote.getDecisionHistory(),
