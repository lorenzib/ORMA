(function(){
  ...
  function activityTitle(item){
    ...
    const stream={dossier:'Evidence decision',content:'Trail content decision',publication:'Release decision','image-publication':'Trail photo release','new-trail':'New Trail decision',hazard:'Groundskeeper decision',editorial:'Editorial decision',image:'Image sourcing decision',newsletter:'Newsletter decision',analyst:'Analyst decision'}[item.stream]||'Workflow decision';
  ...
  function renderActivity(model){
    ...
    for(const item of model.activity){const row=element('li',`bo-activity-item is-${item.status||'processed'}`);const marker=element('span','bo-activity-marker');const body=element('div');body.append(element('small','',activityTitle(item)),element('strong','',item.title),element('p','',item.message));const state=element('span','bo-activity-state',(item.status||'processed').replace(/-/g,' '));row.append(marker,body,state);const published=item.status==='published';const href=published?(item.publicUrl||item.deploymentRunUrl||item.pullRequestUrl):(item.pullRequestUrl||item.workflowRunUrl);if(href){const link=element('a','',published&&item.publicUrl?'Open live trail ↗':item.pullRequestUrl?'Open PR ↗':'Inspect automation run ↗');link.href=href;link.target='_blank';link.rel='noopener';row.append(link);}list.append(row);}
  ...
  async function load(){
    ...
    try{const remote=await api();const [orchestration,dossiers,execution,publication,publicationRequests,workerHealth,campaignHealth,newTrailScouting,newTrailStatus,newTrailReviewResult,hazards,hazardQueue,hazardStatus,hazardReviewResult,editorialPacket1,editorialPacket2,editorialPacket3,editorialReviewResult,editorialReceipts,strategyStatus,imageAudit,imageResults,imagePublicationRequests,imageStatus,imageReviewResult,newsletterPacket,newsletterReviewResult,approvedNewsletters,productIdeas,analystReviewResult,productInvestigations,productDesigns,jobResult,historyResult]=await Promise.all([
      required(remote,'trail-orchestration'),required(remote,'dossier-review-queue'),required(remote,'verified-trail-editorial-execution'),required(remote,'publication-staging'),optional(remote,'publication-requests',{requests:[]}),optional(remote,'worker-health',null),optional(remote,'trail-campaign-health',null),optional(remote,'new-trail-scouting',{candidates:[],summary:{}}),optional(remote,'new-trail-scouting-status',{}),remote.getNewTrailReviews(),optional(remote,'dynamic-hazards',{hazards:[]}),optional(remote,'hazard-review-queue',{items:[]}),optional(remote,'hazard-watch-status',{}),remote.getHazardReviews(),optional(remote,'editorial-review-packet-1',null),optional(remote,'editorial-review-packet-2',null),optional(remote,'editorial-review-packet-3',null),remote.getEditorialReviews(),optional(remote,'editorial-publication-receipts',{receipts:[]}),optional(remote,'strategy-cycle-status',{}),optional(remote,'image-coverage',{gaps:[],summary:{}}),optional(remote,'image-coverage-results',{items:[]}),optional(remote,'trail-image-publication-requests',{requests:[]}),optional(remote,'trail-image-coverage-status',{}),remote.getImageReviews(),optional(remote,'newsletter-review-packet',null),remote.getNewsletterReviews(),optional(remote,'approved-newsletters',{issues:[]}),optional(remote,'product-ideas',{ideas:[]}),remote.getAnalystReviews(),optional(remote,'product-investigation-results',{items:[]}),optional(remote,'product-design-results',{items:[]}),remote.getRevisionJobs(),remote.getDecisionHistory(),
      ...
      if(!editorialReviewResult?.ok)throw new Error(`Could not load Editorial decisions: ${editorialReviewResult?.error||'unknown error'}`);if(!imageReviewResult?.ok)throw new Error(`Could not load image decisions: ${imageReviewResult?.error||'unknown error'}`);
      if(!newsletterReviewResult?.ok)throw new Error(`Could not load Newsletter decisions: ${newsletterReviewResult?.error||'unknown error'}`);if(!analystReviewResult?.ok)throw new Error(`Could not load Analyst decisions: ${analystReviewResult?.error||'unknown error'}`);
      document.getElementById('executiveDecisionQueue').classList.remove('is-error');render(window.ORMADashboardModel.buildDashboardModel({orchestration,dossiers,execution,publication,publicationRequests,workerHealth,campaignHealth,newTrailScouting,newTrailStatus,newTrailReviews:newTrailReviewResult.reviews||[],hazards,hazardQueue,hazardStatus,hazardReviews:hazardReviewResult.reviews||[],editorialPackets:[editorialPacket1,editorialPacket2,editorialPacket3].filter(Boolean),editorialReviews:editorialReviewResult.reviews||[],editorialReceipts,strategyStatus,imageAudit,imageResults,imagePublicationRequests,imageStatus,imageReviews:imageReviewResult.reviews||[],newsletterPacket,newsletterReviews:newsletterReviewResult.reviews||[],approvedNewsletters,productIdeas,analystReviews:analystReviewResult.reviews||[],productInvestigations,productDesigns,jobs:jobResult.jobs||[],history:historyResult.decisions||[]}));seconds=15;const refreshed=new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'});set('dashboardUpdated',`Live · refreshed ${refreshed} · next refresh in ${seconds}s`);
