(function(){
  ...
  function renderNext(model){
    ...
    if(model.handoffsInFlight){owner.textContent='Decision recorded';owner.className='bo-owner-chip is-system';set('nextHandoffTitle','The system is handing your decision forward');set('nextHandoffCopy',`${model.handoffsInFlight} saved decision${model.handoffsInFlight===1?' is':'s are'} waiting for or being consumed by the worker.`);set('nextHandoffAfter','You do not need to click again. The receipt remains below; the trail will return only if another decision is required.');action.hidden=true;return;}
  ...
  function renderActivity(model){
    ...
    for(const item of model.activity){const row=element('li',`bo-activity-item is-${item.status||'processed'}`);const marker=element('span','bo-activity-marker');const body=element('div');body.append(element('small','',activityTitle(item)),element('strong','',item.title),element('p','',item.message));const state=element('span','bo-activity-state',(item.status||'processed').replace(/-/g,' '));row.append(marker,body,state);if(item.pullRequestUrl){const link=element('a','','Open PR ↗');link.href=item.pullRequestUrl;link.target='_blank';link.rel='noopener';row.append(link);}list.append(row);}
