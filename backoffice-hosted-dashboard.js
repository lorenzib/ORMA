(function(){
  ...
  function activityTitle(item){
    ...
    const stream={dossier:'Evidence decision',content:'Trail content decision',publication:'Release decision','new-trail':'New Trail decision',hazard:'Groundskeeper decision',editorial:'Editorial decision',image:'Image sourcing decision',newsletter:'Newsletter decision',analyst:'Analyst decision'}[item.stream]||'Workflow decision';
  ...
  function render(model){
    ...
    set('newsletterProgress',`${model.newsletterProgress.ready} issue ready · ${model.newsletterProgress.inFlight} handoffs · ${model.newsletterProgress.approved} approved issues.`);
  ...
  async function load(){
    ...
    try{const remote=await api();const [orchestration,dossiers,execution,publication,publicationRequests,workerHealth,campaignHealth,newTrailScouting,newTrailStatus,newTrailReviewResult,hazards,hazardQueue,hazardStatus,hazardReviewResult,editorialPacket1,editorialPacket2,editorialPacket3,editorialReviewResult,editorialReceipts,strategyStatus,imageAudit,imageReviewResult,newsletterPacket,newsletterReviewResult,approvedNewsletters,productIdeas,analystReviewResult,productInvestigations,productDesigns,jobResult,historyResult]=await Promise.all([
      ...
      document.getElementById('executiveDecisionQueue').classList.remove('is-error');render(window.ORMADashboardModel.buildDashboardModel({orchestration,dossiers,execution,publication,publicationRequests,workerHealth,campaignHealth,newTrailScouting,newTrailStatus,newTrailReviews:newTrailReviewResult.reviews||[],hazards,hazardQueue,hazardStatus,hazardReviews:hazardReviewResult.reviews||[],editorialPackets:[editorialPacket1,editorialPacket2,editorialPacket3].filter(Boolean),editorialReviews:editorialReviewResult.reviews||[],editorialReceipts,strategyStatus,imageAudit,imageReviews:imageReviewResult.reviews||[],newsletterPacket,newsletterReviews:newsletterReviewResult.reviews||[],approvedNewsletters,productIdeas,analystReviews:analystReviewResult.reviews||[],productInvestigations,productDesigns,jobs:jobResult.jobs||[],history:historyResult.decisions||[]}));seconds=15;const refreshed=new Date().toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'});set('dashboardUpdated',`Live · refreshed ${refreshed} · next refresh in ${seconds}s`);
