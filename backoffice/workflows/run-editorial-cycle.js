...
const {guideInventory,runGuideContent}=require('./run-guide-content');
...
async function runEditorialCycle(root,options={}){
  ...
  for(let index=0;index<selected.length;index++){
    ...
    try{
      const packet=await (options.runGuide||runGuideContent)(root,{guideId:candidate.guideId,at,...(options.runGuideOptions||{})});
      packet.workstream='website-editorial';packet.subject.safetyCritical=candidate.safetyCritical;
      await writeJson(file,packet);
      if(packet.summary.readyForReview){
        ledger=recordEditorialOutcome(ledger,{at,contentId:candidate.contentId,type:'guide',sourceRef:candidate.sourceRef,action:'in-review',contentFingerprint:candidate.contentFingerprint,sourcesFingerprint:sourcesFingerprint(packet),packetFingerprint:packetFingerprint(packet),safetyCritical:candidate.safetyCritical,reviewIntervalDays:42});
...
module.exports={packetFingerprint,sourcesFingerprint,runEditorialCycle};
