...
function applyImageCoverageReview(audit, queue, input, at = new Date().toISOString()){
  ...
  if(['upload-owner-photo','approve-uploaded-photo'].includes(input.action) && !String(input.uploadPath||assetRef||'').startsWith('backoffice/trail-images/')){
    throw new Error('An uploaded trail photo is required');
  }
  const uploadPath=['upload-owner-photo','approve-uploaded-photo'].includes(input.action)?String(input.uploadPath||assetRef||'').trim()||null:null;
  ...
  const decision = {
    slug: gap.slug, trailId:gap.trailId||gap.slug, sourceRef: gap.sourceRef, action: input.action, assetRef, uploadPath,
  ...
  if(input.action !== 'park') jobs.push({
    ...
    assetRef, uploadPath, fileName:decision.fileName,mimeType:decision.mimeType,fileSize:decision.fileSize,
