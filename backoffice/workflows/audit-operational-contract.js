...
async function auditOperationalContract(root=path.resolve(__dirname,'../..'),options={}){
  ...
  add('publication-deployment-receipts',publicationWorkflow.includes('actions: read')&&publicationWorkflow.includes('backoffice:find-pages-deployment')&&publicationWorkflow.includes('backoffice:confirm-publications')&&publicationWorkflow.includes('deployed-site/data/verified-trail-overrides.json')&&deploymentReceiptWorkflow.includes('workflow_dispatch')&&deploymentReceiptWorkflow.includes('backoffice:find-pages-deployment')&&packageManifest.includes('backoffice:find-pages-deployment')&&packageManifest.includes('backoffice:confirm-publications')&&dashboard.includes("status==='published'"),'The fifteen-minute worker verifies a successful Pages run for the exact deployed commit, matches deployed approval IDs, records a protected published receipt, clears the PR gate and exposes the live trail link; the same verifier remains manually recoverable');
  ...
  add('worker-cadence',workflows['orma-backoffice-worker.yml']?.includes("cron: '*/15 * * * *'")&&workflows['orma-backoffice-worker.yml']?.includes('ORMA_WORKER_AUTOMATION_ENABLED'),'The queue worker has a fifteen-minute target and an activation variable');
