...
async function auditOperationalContract(root=path.resolve(__dirname,'../..'),options={}){
  ...
  const hostedPages=['trail-dossier-desk.html','trail-content-desk.html','image-coverage-desk.html','community-moderation-desk.html'];
  add('backfill-hosted-desks',hostedPages.every(page=>builder.includes(`'${page}'`))&&!builder.includes("'editorial-desk.html'")&&!builder.includes("'newsletter-desk.html'")&&!builder.includes("'product-ideas-desk.html'")&&!builder.includes("'designer-desk.html'")&&!builder.includes("'hazard-review-desk.html'")&&!builder.includes("'new-trail-scouting-desk.html'"),`${hostedPages.length} protected desk routes are allowlisted and the retired lanes are gone`);
