...
async function auditOperationalContract(root=path.resolve(__dirname,'../..'),options={}){
  ...
  add('publication-deployment-receipts',deploymentReceiptWorkflow.includes('workflow_run')&&deploymentReceiptWorkflow.includes('pages build and deployment')&&deploymentReceiptWorkflow.includes('deployed-site/data/verified-trail-overrides.json')&&deploymentReceiptWorkflow.includes('backoffice:confirm-publications')&&packageManifest.includes('backoffice:confirm-publications')&&dashboard.includes("status==='published'"),'Successful Pages deployment matches the exact deployed approval IDs, records a protected published receipt, clears the PR gate and exposes the live trail link');
  add('truthful-team-gates',standard.includes('Approval hands the issue to Social')&&standard.includes('An investigation, priority decision, or mock-up never authorises development'),'Newsletter sending, Analyst implementation and Release remain explicitly gated');
