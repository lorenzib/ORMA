...
describe('hosted weekly strategy cycle',()=>{
  test('stores three protected copy packets, image audit, and a truthful health receipt',async()=>{
    ...
    try{const result=await runLiveStrategyCycle(store,{root,at:'2026-08-20T12:00:00Z',completedAt:'2026-08-20T12:10:00Z',runId:'123',runEditorialCycle,auditImageCoverage:async()=>({summary:{pagesScanned:11,missing:8},gaps:[]})});expect(result.summary).toEqual(expect.objectContaining({editorialActive:3,imageGaps:8}));expect(artifacts['strategy-cycle-status']).toEqual(expect.objectContaining({status:'healthy',runId:'123',publicMutationAllowed:false}));expect(artifacts['editorial-review-packet-3']).toBeTruthy();}
