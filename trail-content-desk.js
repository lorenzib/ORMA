(function(){
  ...
  async function loadRevisionJobs(){if(LOCAL_MODE)return getJson(URLS.revisions);const api=await waitForRemoteBackoffice();const result=await api.getRevisionJobs();if(!result.ok)throw new Error(`Could not load revision jobs: ${result.error}`);return {contractVersion:'1.0.0',jobs:result.jobs.filter(job=>['verified-trail-editorial-first-pass','verified-trail-editorial-revision'].includes(job.jobType)||String(job.id||'').startsWith('trail-revision-'))};}
  ...
  function renderRevisionActivity(){
    ...
    jobs.forEach(job=>{const item=element('li',`bo-revision-job is-${job.status}`);const trail=queue.items.find(candidate=>candidate.candidateId===job.candidateId);const firstPass=job.jobType==='verified-trail-editorial-first-pass';const head=element('div','bo-revision-job-head');head.append(element('strong','',`${trail?.trailName||job.candidateId} · ${job.agentId}${firstPass?' · first pass':` · revision ${job.attempt||1}`}`),element('span','bo-content-status',job.status.replace(/-/g,' ')));item.append(head,element('p','',firstPass?(job.action||'Prepare first proposal'):`“${job.instruction}”`),element('small','',`Queued ${dateValue(job.createdAt).toLocaleString()}${job.completedAt?` · completed ${dateValue(job.completedAt).toLocaleString()}`:''}`));if(job.auditSummary)item.append(element('p','',job.auditSummary));if(job.resolution)item.append(element('p','',job.resolution));if(job.status==='ready-for-review'){const open=element('a','bo-source-link',firstPass?'Evaluate proposal ↓':'Evaluate revision ↓');open.href=`#trail-${job.candidateId}`;item.append(open);}revisionJobs.append(item);});
  }
  ...
  function reviewControls(output){
    ...
    [['approve','Approve'],['request-revision','Request revision'],['reject','Reject'],['clear','Clear']].forEach(([action,label])=>{
      const selected=decisions[output.jobId]?.action===action||receipt?.decision.action===action;const missingReadyAsset=action==='approve'&&output.agentId==='visualDirector'&&(output.result?.candidates||[]).filter(candidate=>candidate.status==='ready').length!==1;const button=element('button',selected?'is-selected':'',label);button.type='button';button.dataset.action=action;button.disabled=locked||missingReadyAsset;if(missingReadyAsset)button.title='Exactly one fully licensed ready image is required before approval';
  ...
  function lockedEvidence(item){const details=element('details','bo-raw-evidence');details.append(element('summary','',`Inspect ${item.lockedFacts?.length||0} locked facts and verification conditions`));const facts=element('dl','bo-evidence-fields');(item.lockedFacts||[]).forEach(fact=>facts.append(element('dt','',`${fact.id} · ${fact.label}`),element('dd','',fact.value)));details.append(facts);if(item.verificationConditions?.length){const conditions=element('div','bo-open-questions');conditions.append(element('h4','','Verification conditions'));const list=element('ul');item.verificationConditions.forEach(condition=>list.append(element('li','',condition)));conditions.append(list);details.append(conditions);}const sources=element('div','bo-dossier-sources');(item.evidenceSources||[]).forEach(source=>{if(/^https:\/\//.test(source.url||''))sources.append(link(`${source.label}${source.authority?` · ${source.authority}`:''}`,source.url));});if(sources.children.length)details.append(sources);return details;}
  function renderPackets(){
    packets.replaceChildren();
    queue.items.forEach(item=>{const card=element('article','bo-trail-packet');card.id=`trail-${item.candidateId}`;const head=element('div','bo-content-head');const intro=element('div');intro.append(element('p','eyebrow','ORMA Verified trail'),element('h2','',item.trailName),link('Open trail evidence desk',item.dossierRef));head.append(intro,element('span','bo-content-status','Trail only'));card.append(head,lockedEvidence(item));
  ...
  function renderStaging(next){
    staging=next;publicationSummary.replaceChildren();[['Trails',staging.summary.trails],['Ready for preview',staging.summary.readyForPreview],['Waiting for approvals',staging.summary.waitingForApprovals],['Waiting for mapping',staging.summary.waitingForMapping||0],['Public mutations',staging.summary.publicMutations]].forEach(([label,value])=>publicationSummary.append(element('span','',`${label}: ${value}`)));
    publicationMapper.replaceChildren();staging.items.forEach(item=>{const source=queue.items.find(candidate=>candidate.candidateId===item.candidateId);const card=element('article',`bo-publication-card is-${item.state}`);card.id=`publication-${item.candidateId}`;card.append(element('h3','',source?.trailName||item.targetTrailId),element('p','bo-content-status',item.state.replace(/-/g,' ')));
      if(item.state!=='ready-for-publication-preview'){const required=[...(item.missingApprovals||[]),...(item.publicationMappingBlockers||[])];card.append(element('p','bo-empty',`Still required: ${required.join(', ')||'a complete publication mapping'}. ${item.missingApprovals?.length?'Review both proposals above and submit those decisions first.':'The content decisions are retained; ORMA must prepare the structured website mapping before release approval can open.'}`));}
