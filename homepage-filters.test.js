...
describe('map-first returning homepage layout contract', () => {
  ...
  test('keeps route shields above a stronger ORMA highlight and only clusters five or more trails', () => {
    ...
    expect(script.match(/7, 0\.48/g)).toHaveLength(2);
    expect(script.match(/10, 0\.66/g)).toHaveLength(2);
    expect(script.match(/12, 0\.84/g)).toHaveLength(2);
    expect(script.match(/14, 1/g)).toHaveLength(2);
    expect(script.match(/'raster-saturation': 0/g)).toHaveLength(2);
    expect(script.match(/'raster-contrast': 0/g)).toHaveLength(2);
    ...
    expect(trailScript).toContain("'raster-saturation': 0");
    expect(trailScript).toContain("'raster-contrast': 0");
  });
});
