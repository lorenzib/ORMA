...
describe('map-first returning homepage layout contract', () => {
  ...
  test('keeps route shields above a stronger ORMA highlight and only clusters five or more trails', () => {
    ...
    expect(trailScript).toContain("9, 0.50");
    expect(trailScript).toContain("12, 0.65");
    expect(trailScript).toContain("14, 0.88");
    expect(trailScript).toContain("15, 0.96");
    expect(trailScript).toContain("'raster-saturation': -0.72");
    expect(trailScript).toContain("'raster-contrast': 0.36");
  });
});
